...
//Grid Component
const Grid = ({ GridData, setUpObstacles, setDraggedNode, sourceNode, destinationNode, visitedNodes, pathNodes }) => {
        ...
        <div className="grid"  >
            {
                Object.keys(GridData).map((node, i) => {
                    return <div className={visitedNodes[node] ? `node visited ${GridData[node].toLowerCase()} ${pathNodes[node] ? `path` : ''}  ` : `node  ${GridData[node].toLowerCase()}  ${pathNodes[node] && `path`}`} key={i} title={"node: " + node} onMouseEnter={() => setObstacle(node)} onMouseDown={() => initiateObstacleSetup(node)} onMouseUp={() => setMouseHold(false)}    >
                        {
